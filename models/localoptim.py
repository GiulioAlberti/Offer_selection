...
class LocalOptim:
    ...
    def __init__(self, flights):
        ...
        self.x = self.p.addVars([(flight.slot.index, j.index) for j in self.slots for flight in flights], vtype=GRB.BINARY)

    def set_constraints(self):
        for flight in self.flights:
            self.p.addConstr(quicksum(self.x[flight.slot.index, j.index] for j in self.slots) == 1, name="c1[%s]" % flight)
            self.p.addConstr(
                quicksum(self.x[flight.slot.index, j.index] for j in self.slots if j.time < flight.eta) == 0,
                name="c2[%s]" % flight)

        for j in self.slots:
            self.p.addConstr(quicksum(self.x[flight.slot.index, j.index] for flight in self.flights) <= 1, name="c3[%s]" % j)
