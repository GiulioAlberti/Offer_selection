...
class OfferEval:
    ...
    def set_objective(self):
        self.p.setObjective(
            quicksum(self.x[flight.index, j] * flight.normCostVect[j] for flight in self.offer.flights_both for j in
                     self.indexes))
    ...
    def solve(self):
        ...
        if status == GRB.OPTIMAL:
            cost_reduction = sum(
                flight.normCostVect[flight.index] for flight in self.offer.flights_both) - self.p.objVal
